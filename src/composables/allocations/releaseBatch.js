...
export async function releaseDraftsBatch(drafts) {
    ...
    for (const r of drafts) {
        ...
        const draft = draftFiles.find(f => f.name === filename);
        if (!draft)
            continue;
        const raw = await readJSON(draft.id);
        if (!raw || !Array.isArray(raw.allocations))
            continue;
