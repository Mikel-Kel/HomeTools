...
async function reopenReleasedIfNeeded(spendingId, draftsFolder, releasedFolder) {
    ...
    console.info(`🔁 Allocation ${spendingId} moved from released → drafts`);
}
/* =========================
   Composable
========================= */
export function useAllocation(spendingId, spendingAmount) {
    ...
    const canSaveDraft = computed(() => isBalanced.value);
    const canRelease = computed(() => isBalanced.value && hasDraft.value);
    /* =========================
       Allocation editing
    ========================= */
    ...
    async function loadDraft() {
        if (draftLoaded.value || !driveState.value)
            return;
        busy.value = true;
        try {
            ...
            if (!file) {
                ...
                if (releasedFile) {
                    const raw = await readJSON(releasedFile.id);
                    // écrire en drafts
                    await writeJSON(draftsFolder, filename, raw);
                    // supprimer released
                    await deleteFile(releasedFile.id);
                    // relister drafts
                    const updatedDrafts = await listFilesInFolder(draftsFolder);
    ...
    async function saveDraft() {
        if (!canSaveDraft.value || !driveState.value)
            return;
        busy.value = true;
        ...
        try {
            ...
            await writeJSON(folder, filename, {
                version: 1,
                spendingId,
    ...
    async function release() {
        if (!canRelease.value || !driveState.value)
            return;
        busy.value = true;
        ...
        try {
            ...
            ───────────────────────────── */
            const releasedFiles = await listFilesInFolder(releasedFolder);
            const existingReleased = releasedFiles.find(f => f.name === filename);
            /* ─────────────────────────────
